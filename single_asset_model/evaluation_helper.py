...
def graph(data, stock_ticker, model_version, run_number, alpha, mu):
    ...
    ax[0].plot(data['date'],data['wealth'], label='Linear Model', color='m')
    ...
    ax[1].axhline(y =mu, color = 'r', linestyle = '--', label = 'Historical Mean')
    ax[1].legend(loc='upper left')

    ax[2].plot(data['date'],data['portfolio_balance_trained'], label='Trained Model Position', color='g')
    ax[2].plot(data['date'],data['portfolio_balance_base'], label='Base Model Position', color='b')
    ax[2].plot(data['date'],data['portfolio_balance'], label='Linear Model Position', color='m')
    ax[2].axhline(y=0, color='r', linestyle='--', label='No Position')
    ...
    ax[0].set_title('Single Asset Model Evaluation for ' + stock_ticker + ' with alpha = ' + str(alpha))

    plt.tight_layout()
    plt.savefig(f"evaluation_graphs/{stock_ticker}_evaluation_model_{model_version}_run_{run_number}.png")
    plt.close()